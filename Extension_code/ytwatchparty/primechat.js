...
let videoContainer = document.getElementById("dv-web-player");
let file;
let count = 0;
...
<div class="gif_box" id="gif_box_parent"  style="display:none";  >  <input id="input_meme"  style="padding: 4px;width: 100%;" placeholder="Search Gif..."></input>            
<div  id="gif-box"> 
</div>
...
let inteval = setInterval(() => {
  ...
  const continuebutton2 = document.querySelector('[aria-label="Play"]');
  const continuebutton3 = document.querySelector(
    '[aria-label="Watch again"]'
  );
  videoContainer = document.getElementById("dv-web-player");
                   
  let fullscreen = document.querySelectorAll(".dv-player-fullscreen")[0];

  if (continuebutton) {
    flag2 = true;
    document.querySelector('[aria-label="Continue watching"]').click();
  ...
  if(continuebutton3){
    document.querySelector('[aria-label="Watch again"]').click();
  }
  if (
    fullscreen &&
    videoContainer &&
...
function init() {
  ...
  //   maincontainer.appendChild(chat_box);
