console.log('jio init......')

console.log('run hotstarrrr')
...
if(document.querySelector('video') &&  document.getElementById('player-wrapper')){
  inntilize();
  clearInterval(interval);
}
...
function inntilize() {
  ...
  const myVideo = document.querySelectorAll("video")[0];
  let globalFlag = true;
  ...
  socket.on("videoEvent", (event) => {
    console.log(event.type,"recived...")
    switch (event.type) {
      case "PLAY":
